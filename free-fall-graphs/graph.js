...
function Graph(context,xmin,xmax,ymin,ymax,x0,y0,xwidth,ywidth) {
  ...
  var x_orig;	var y_orig;
  // scaling used in displaying values on the axes
  var x_displ_scal;	var y_displ_scal;

  // overall width and height of graph in pixels
  var x_width;	var y_width;
  // min and max of x and y relative to origin (in pixels)
  var x_min_rel;	var x_max_rel;	var y_min_rel;	var y_max_rel;
  // obvious
  var x_tick_major;	var x_tick_minor;	var y_tick_major;	var y_tick_minor;
  ...
  var x_min;	var x_max;	var y_min;
  var y_max;	var xx;	var yy;
	var x_displ;	var y_displ;	var txpos;	var typos;

  // PARAMETER ASSIGNMENTS // assign parameter values based on specified arguments
  x_orig=x0;	y_orig=y0;	x_width=xwidth;	y_width=ywidth;
  //
  x_displ_scal=(xmax-xmin)/xwidth;	y_displ_scal=(ymax-ymin)/ywidth;
  //
  x_min_rel=xmin/x_displ_scal;	x_max_rel=xmax/x_displ_scal;
  y_min_rel=ymin/y_displ_scal;	y_max_rel=ymax/y_displ_scal;

  // convert to absolute coordinates
  x_min=x_min_rel + x_orig;	x_max=x_max_rel + x_orig;
  y_min=y_orig - y_min_rel;	y_max=y_orig - y_max_rel;
  txpos=x_orig - tw;	typos=y_orig;
  // METHODS
  ...
  this.drawgrid = function(xmajor,xminor,ymajor,yminor){
    ...
    do {
        ctx.moveTo(xx,y_min);
        ctx.lineTo(xx,y_max);	xx+= x_tick_major;
      } while (xx <= x_max);
    ctx.stroke();
    ...
    do {
      ...
      ctx.fillText(Math.round(y_displ),txpos + 5,yy - th / 2);
      yy+= y_tick_major;
  } while (yy <= y_min);
  ctx.textAlign = "left";
  ctx.textBaseline = "top"; xx=x_min;
  do {
    x_displ=(xx - x_orig) * x_displ_scal;
    ctx.fillText(Math.round(x_displ),xx - tw + 10,typos + 5);
    xx+= x_tick_major;
  } while (xx <= x_max); };
...
this.drawaxes = function (xlabel,ylabel){
  ...
  ctx.fillText(xlabel,x_max/2 + 0.75 * tw,typos + th /2);
  ctx.fillText(ylabel,txpos,ywidth/2);
  */
};
// PLOT DATA: plot data
this.plot = function (xArr, yArr, pColor, pDots, pLine){
  ...
  for (var i=1; i<xArr.length; i++){
      xpos=x_orig+xArr[i]/x_displ_scal;	ypos=y_orig-yArr[i]/y_displ_scal;
      if (xpos<x_max && ypos>y_max){
  	     if (pLine){
